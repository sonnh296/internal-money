...
import com.account.management.mapper.AccountMapper;
import com.account.management.model.*;
import com.account.transaction.model.*;
import com.account.transaction.service.*;
import com.account.management.repository.AccountHoldRepository;
...
import com.account.transaction.mapper.TransactionMapper;
...
public class AccountService {
	...
	public AccountResponse get(UUID id) {
		return mapper
				.toDto(accountRepo.findById(id).orElseThrow(() -> new com.commons.exception.AccountNotFoundException(id)));
	}

	public AccountBalanceResponse getBalance(UUID id) {
		Account a = accountRepo.findById(id).orElseThrow(() -> new com.commons.exception.AccountNotFoundException(id));
	...
	AccountResponse creditInTransaction(UUID id, PostingRequest r, Integer expectedVersion, String idempotencyKey) {
		Account a = accountRepo.findByIdForUpdate(id)
				.orElseThrow(() -> new com.commons.exception.AccountNotFoundException(id));
		ensureOwnerOrAdmin(a);

		if (idempotencyKey != null && !idempotencyKey.isBlank()) {
			String fp = idempotencyKey.trim();
			Optional<com.account.transaction.model.Transaction> existing = transactionService.findByAccountAndFingerprint(id, fp);
			if (existing.isPresent()) {
		...
		if (expectedVersion != null && !expectedVersion.equals(a.getVersion())) {
			throw new com.commons.exception.VersionMismatchException("ETag mismatch");
		}
	...
	InternalTransferResponse transferInTransaction(InternalTransferRequest request, String idempotencyKey) {
		...
		if (request.fromAccountId().equals(target.getId())) {
			throw new com.commons.exception.BadRequestException("Source and destination account must be different");
		}
		...
		if (replay.isPresent()) {
			Account from = accountRepo.findById(request.fromAccountId())
					.orElseThrow(() -> new com.commons.exception.AccountNotFoundException(request.fromAccountId()));
		...
		Account first = accountRepo.findByIdForUpdate(firstLockId)
				.orElseThrow(() -> new com.commons.exception.AccountNotFoundException(firstLockId));
		Account second = accountRepo.findByIdForUpdate(secondLockId)
				.orElseThrow(() -> new com.commons.exception.AccountNotFoundException(secondLockId));
		...
		if (from.getStatus() != AccountStatus.ACTIVE || to.getStatus() != AccountStatus.ACTIVE) {
			throw new com.commons.exception.InvalidTransitionException(
		...
		if (!from.getCurrency().equalsIgnoreCase(to.getCurrency())) {
			throw new com.commons.exception.BadRequestException(
	...
	HoldResponse createHoldInTransaction(UUID accountId, CreateHoldRequest r) {
		Account a = accountRepo.findByIdForUpdate(accountId)
				.orElseThrow(() -> new com.commons.exception.AccountNotFoundException(accountId));
		ensureOwnerOrAdmin(a);
	...
	AccountResponse captureHoldAndDebitInTransaction(UUID accountId, UUID holdId, PostingRequest r,
		...
		Optional<com.account.transaction.model.Transaction> existing = transactionService.findByAccountAndFingerprint(accountId, fp);
		if (existing.isPresent()) {
		...
		AccountHold h = holdRepo.findById(holdId)
				.orElseThrow(() -> new com.commons.exception.ResourceNotFoundException("hold", holdId));
		if (!h.getAccountId().equals(accountId)) {
			throw new com.commons.exception.BadRequestException("Hold does not belong to this account");
		}
		...
		for (int attempt = 1; attempt <= MAX_OPTIMISTIC_RETRIES; attempt++) {
			try {
				...
				if (attempt >= MAX_OPTIMISTIC_RETRIES) {
					throw new com.commons.exception.VersionMismatchException(
	...
	private static void sleepBackoff(int attempt) {
		try {
			...
			throw new com.commons.exception.VersionMismatchException("Giao dịch bị gián đoạn");
		}
	}
