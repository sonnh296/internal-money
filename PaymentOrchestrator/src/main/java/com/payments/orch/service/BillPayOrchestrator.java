...
public class BillPayOrchestrator {
   ...
   * Redeem điểm được thực hiện ở StatusConsumer khi billing POSTED.
   */
  public PaymentAcceptedResponse acceptBillPay(BillPayRequest req, String idemKey) {
    ...
    long pointsToRedeem = req.pointsToRedeem() == null ? 0L : req.pointsToRedeem();
    ...
    try {
      return paymentTransactionTemplate.execute(status ->
          persistPaymentAndOutbox(req, idemKey, holdId, holdAmount, pointsToRedeem));
  ...
  private PaymentAcceptedResponse persistPaymentAndOutbox(
      BillPayRequest req, String idemKey, UUID holdId, BigDecimal holdAmount, long pointsToRedeem) {
